...
\chapter{Presentation of IVy}
    ...
    \begin{itemize}
        ...
        \item A list of axioms over these variables, functions and relations. Axioms are \(\exists^*\forall^*\) formulas (\( \phi_{EA} \) in the grammar below).
        \item A list of transitions that we call `actions'. An action is a \(statement\) (see the grammar below).
    ...
    \begin{enumerate}
        \item \(I\) is satisfied just after the initialization of the program.
        ...
        \item Actions never abort abnormally from a state satisfying \(I\).
    \end{enumerate}
