...
\chapter{My contributions}
    ...
    \begin{algorithm}[H]
        \SetKwFunction{RPM}{Retropopagate_marks}%
        \SetKwProg{Fn}{Function}{}{end}
        \SetKwSwitch{Match}{Case}{Other}{match}{}{case}{otherwise}{endcase}{end}
        \Fn{\RPM{M',S,E,E'}}{
            \KwData{a set of marks \(M'\) after the execution of the statement \(S\), an environment \(E\) and \(E'\) respectively before and after the execution of \(S\)}
            \KwResult{a new set of marks \(M\) such that every state concurring with \(E\) on \(M\)
            will concur with \(E'\) on \(M'\)}
    ...
    In this way, the advantages of the two methods are cumulated. Moreover, the unSAT core to minimize will be smaller, so it will be done faster.
