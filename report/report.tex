...
\chapter{Invariant synthesis}
    ...
    % TODO: add constraints of the form (val /= val) to this set of constraints instead of adding these constraints later. Also apply it to other occurences in the document.
    % Also give a name to this set of constraints (like constr(C))
    ...
    \begin{itemize}
        ...
        aggregates all constraints of the initial state \(S\) of the counterexample, we are able to only keep a minimal number of constraints
    ...
    It is the method currently implemented by IVy in order to help the user to synthesize inductive invariants interactively.
...
\chapter{My contributions}
    ...
    \begin{enumerate}
        ...
        \item We try to compute a subset of \(C'\) that is sufficient to guarantee that the current invariant is broken.
        More precisely, we compute a set of `marks', a `mark' being a pointer to an important constraint.
        A mark is an object of the form \(symbol(concrete\_value_1,\ldots,concrete\_value_n)\). % TODO
    ...
    We will see in the next section how to `generalize' this state and use it to weaken the generated invariant.
    ...
    First, we can compute the set \(C'\) of the concrete constraints characterizing the interpretations of \(S'\).% TODO
