...
\chapter{My contributions}
    ...
    \begin{enumerate}
        \item \(S'\) satisfies all constraints \(C\). It implies that the domains of \(S'\) contain all concrete values that appear in \(C\).
        However, they can also contain some other elements.
        \item \(S'\) is a valid state (it can be reached with a valid run).
    \end{enumerate}
    ...
    \(S'\) satisfies the current invariant and the axioms, and doesn't break any of them after one iteration (in particular after executing the action that is executed in the counterexample).
...
% TODO: rename S' in Sv
    ...
    we will generalize it and we will build a new invariant that allows valid states that are similar to \(S'\).
    ...
    First, we can compute a subset \(C'\) of \(constr(S')\) by computing marks for some relevant executions starting in \(S'\)
    ...
    We want to characterize states that satisfy \(C\), but we only want invalid states, so we don't want to include states that satisfy \(F'\).
