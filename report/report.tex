...
\chapter{My contributions}
    ...
    If the conditions 1 and 2 are satisfiable, we can compute a finite model: it gives us a state as desired.
    We will see in the next section how to `generalize' the constraints characterizing this state and use them to weaken the generated invariant.
    We can then try to find an other such state, by adding as a third condition the negation of these generalized constraints.
    This process can potentially not terminate if we always find new such states: we will discuss it in the section 3.5.
