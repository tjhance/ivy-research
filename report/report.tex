...
\chapter{Presentation of IVy}
    ...
    \begin{itemize}
        \item A list of uninterpreted types that we call `sorts'. A sort has an unbounded number of elements (`values').
        ...
        \item A list of axioms over these variables, functions and relations. Axioms have the form \( \phi_{EA} \) in the grammar below.
        \item A list of transitions that we call `actions' (see the grammar of \(statement\) below).
    ...
    Note that the formulas \( \phi_{EA} \) are subject to some additional restrictions that will be detailled in section 1.3.
    ...
    Also, unlike many other languages like F*, IVy does not allow refinement types or dependent types (for decidability reasons).
    ...
    We will refer to it as EPR (for Effectively Propositional) because this logic can be effectively translated into propositional logic (see the subsection 1.3.2).
        ...
        In IVy, relations, functions and constants (that are preferably called `variables' since they are mutable) are typed.
    ...
    % Table with:
    % Semantic of statements & syntactic sugars
    % How to compute their wpr
    ...
    \begin{tabular}{|l|l|l|}
        ...
        \hline
        skip & Do nothing & \(Q\) \\
        abort & Terminate abnormally & false \\
        \textbf{r}(\(\bar{x}\)) := \( \phi_{QF}(\bar{x}) \) & Quantifier-free update & \((A \rightarrow Q) [\phi_{QF}(\bar{s})\ /\ \text{\textbf{r}}(\bar{s})]\) \\
        \hline
    \end{tabular}
