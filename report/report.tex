...
\chapter{My contributions}
    ...
    \begin{enumerate}
        \item We compute the state \(S'\) of the counterexample after the execution of \(E\). We call `environment' the set of all constraints
    ...
    To performs the step 2, we can use the following algorithm (a `value' is a term or a formula):

    \begin{algorithm}[H]
        \SetKwFunction{MFV}{Marks_for_value}%
        \SetKwProg{Fn}{Function}{}{end}
        \SetKwSwitch{Match}{Case}{Other}{match}{}{case}{otherwise}{endcase}{end}
        \Fn{\MFV{E,V}}{
            \KwData{an environment \(E\), a value \(V\)}
            ...
            \Match{V}{
                \uCase{symbol(\(V_1,\ldots,V_n\))}{
                    Compute \(M_1,\ldots,M_n\) the marks for values \(V_1,\ldots,V_n\)\;
        ...
        \caption{Computing marks for a value}
    \end{algorithm}

    For instance, for the formula ...% TODO
