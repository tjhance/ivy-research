...
bindingoffset=10mm, top=24mm,bottom=24mm} %top=25mm,bottom=25mm
...
\chapter{Presentation of IVy}
    ...
    \begin{itemize}
        ...
        \item A set of safety properties, that is first order formulas that should be satisfied after executing any action in a valid run (starting with the `init' action).
        Only a fragment of first order logic can be used for decidability reasons (see sections 1.3 and 1.4).
    \end{itemize}
    ...
    and safety properties (=invariants).
...
\chapter{Invariant synthesis}
    ...
    and try to sthrengthen them by adding new invariants (using one of the methods described in the next sections) until this set of invariants form an inductive invariant
    (thereafter, we will use the terms `invariant' or `set of invariants' indistinguishably: a set of invariants can be seen as a single invariant that is the conjunction of all invariants of the set).
    ...
    given an invariant that is not inductive, we want to sthrengthen it (as much as possible) without breaking the correctness
    of the invariant nor the decidability of checking it. To be correct, an invariant must be satisfied by every state
    that can be obtained after a valid execution, that is after executing the `init' action followed by any other actions.
    ...
    \begin{enumerate}
        ...
        \item \textbf{Decidability}: Is the new invariant generated checkable in a decidable way (in our case, we want it to be EPR)?
    ...
    \textbf{Reliability}: This method always terminate and never fail. Moreover, it produces correct invariants. Here is a proof:
    ...
    \begin{itemize}
        \item There exists an execution \(E'\) of one iteration (one action) starting in \(S\) such that the resulting state doesn't satisfy the initial invariant.
    ...
    \begin{definition}
        ...
        \begin{itemize}
            ...
            being any elements of the domains of \(S\)
            \item \(concrete\_value_1 \neq concrete\_value_2\) (with `\(concrete\_value_1\)' and `\(concrete\_value_2\)' of the same type)
        \end{itemize}
    \end{definition}

    We can generate a formula as follows:
    \begin{enumerate}
        \item We associate a variable \( X_v \) to each concrete value \(v\) in \(S\)
        \item For each constraint in \(constr(S)\), we replace each concrete value \(v\) by \( X_v \)
    ...
    The resulting formula \(F\) is an existentially quantified formula that characterizes all structures containing a substructure that is isomorphic to the structure of the counterexample.
    ...
    This technique always generate an universal invariant, and so there is no decidability issue.
    ...
    \begin{itemize}
        ...
        aggregates all constraints in \(constr(S)\), we are able to only keep a minimal number of constraints
    ...
    This method seems less reliable than the weakest precondition, however it generates invariants that are much more interesting.
...
\chapter{My contributions}
    ...
    we want to highlight a subset of \(constr(S)\) (see section 2.3) as small as possible, such that there is no valid state (= state that can be reached with a valid run) that can satisfy these constraints
    ...
    \begin{enumerate}
        ...
        evaluate the current invariant to the same value. More precisely, we compute a set of `marks', a `mark' being a pointer to an important constraint:
        it can be an object of the form \(symbol(concrete\_value_1,\ldots,concrete\_value_n)\) or \(concrete\_value_1 \neq concrete\_value_2\).
        \item We retropopagate these marks through the execution of \(E\) in order to obtain a subset \(C\) of \(constr(S)\) such that
        all states satisfying \(C\) (modulo renaming of the concrete values) can reproduce the execution \(E\) and satisfy \(C'\) at the end.
    \end{enumerate}
    ...
    \begin{algorithm}[H]
        ...
        \Fn{\MFV{S,V}}{
            ...
            \KwResult{a set of marks \(M\) that points to some constraints in \(constr(S)\)}
    ...
    \begin{algorithm}[H]
        ...
        \Fn{\RPM{M',E,S}}{
            ...
            \KwResult{a new set of marks \(M\) that points to some constraints in \(constr(S)\)}
    ...
    Such states would be the witnesses of the non-correctness of our generated invariant.
    ...
    \begin{enumerate}
        \item \(S_v\) satisfies all constraints \(C\). It implies that the domains of \(S_v\) contain all concrete values that appear in \(C\).
        However, they can also contain some other elements.
        \item \(S_v\) is a valid state (it can be reached with a valid run).
    \end{enumerate}
    ...
    We will see in the next section how to produce a formula that `generalize' this state and use it to weaken the generated invariant.
    By adding as a third condition the negation of this formula, an other valid state can be searched.
    ...
    \begin{itemize}
        ...
        \item To generate a valid invariant from a counterexample, we first generate an universal invariant and we weaken it until there are no more valid states satisfying it.
    \end{itemize}
    ...
    we will generalize it and we will build a new invariant that allows valid states that are similar to \(S_v\).

    The process of generalization of \(S_v\) is quite similar to the process of generalization of the counterexample \((S,E)\) (see section 2.3).
    ...
    Then, we remove from \(C_v\) every constraint that already is in \(C\) in order to keep only constraints that are specific to \(S_v\).

    Then, we replace each different concrete value in \(C_v\) by a fresh existentially quantified variable, but only for concrete values that are specific to \(S_v\):
    ...
    contains a substructure isomorphic to \(S_v\). Note that, again, it may exist some states that satisfy \(F_v\) but that are not
    valid (for the same reason that there exists states such as \(S_v\) that satisfy all the constraints in \(C\) but that are valid).
    ...
    We want to characterize states that satisfy \(C\), but we only want invalid states, so we don't want to include states that satisfy \(F_v\).
    ...
    (except if we use symbolic bounded verification to reduce the set of constraints \(C\)).
    However, if there is no universal invariant that can capture the counterexample, this algorithm might not terminate (see section 3.3).\\

    \textbf{Decidability}: If there exists an universal invariant that can capture the counterexample, then such an invariant will be produced.
    ...
    % What remains to be done: optimisation, etc
