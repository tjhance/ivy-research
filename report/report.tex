...
\chapter{Invariant synthesis}
    ...
    The synthesis of inductive invariants is a common problem in verification, and even with the restrictions of RML, it is still undecidable (though it can be decidable for some very restricted classes of invariants). [REF]
...
\chapter{My contributions}
    ...
    can be obtained through a valid execution (for non-monotonic counterexamples it is not always possible, but it will be treated in section 3.3).

    To solve this problem, the IVy approach is to compute a minimal unSAT core, by using symbolic bounded verification to characterize valid runs.
    ...
    \begin{itemize}
        ...
        \item Even if the code is non-monotonic and no universal invariant exists, it can still gather some important constraints.
        From them, a valid invariant of the form \(\forall^*\exists^*\) can sometimes be generated, as we will see in the section 3.3.
    \end{itemize}
